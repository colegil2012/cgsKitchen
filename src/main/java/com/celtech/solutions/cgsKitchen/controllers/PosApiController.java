...
public class PosApiController {
    ...
    public ResponseEntity<Order> updateStatus(@PathVariable String id,
        ...
        var status = Order.Status.valueOf(body.getOrDefault("status", "PAID"));
        return orderService.findById(id)
                .map(o -> {
                    o.setStatus(status);
    ...
    public Map<String, String> paymentIntent(@Valid @RequestBody IntentRequest req)
        ...
        if (props.stripe().platformFeeBps() > 0) {
            long fee = req.amount() * props.stripe().platformFeeBps() / 10_000;
            paramsBuilder.setApplicationFeeAmount(fee);
        }
